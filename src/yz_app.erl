...
maybe_setup(true, SupPid) ->
    ...
    maybe_register_pb(RSEnabled, ?QUERY_SERVICES),
    maybe_register_pb(RSEnabled, ?ADMIN_SERVICES),
    riak_core_node_watcher:service_up(yokozuna, SupPid),
    ok.

%% @doc Conditionally register PB service IFF Riak Search is not
%%      enabled.
-spec maybe_register_pb(boolean(), list()) -> ok.
maybe_register_pb(true, _) ->
    ok;
maybe_register_pb(false, Services) ->
...
%% Search is disabled.
%%
%% Previous versions of Riak had a bucket in `set_bucket' which caused
...
%% this hook will be in the raw ring.  After migrating to Yokozuna
%% these hooks must be removed to avoid errors.
